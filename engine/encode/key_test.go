...
func testMakeKey(t *testing.T, key []engine.ColumnKey, values []sql.Value,
...
func TestMakeKey(t *testing.T) {
	...
	testMakeKey(t, []engine.ColumnKey{engine.MakeColumnKey(0, false)}, values,
		func(val sql.Value) []sql.Value {
			return []sql.Value{val}
		})

	for _, val0 := range values {
		testMakeKey(t, []engine.ColumnKey{
	...
	testMakeKey(t, []engine.ColumnKey{engine.MakeColumnKey(0, true)}, reverseValues,
		func(val sql.Value) []sql.Value {
			return []sql.Value{val}
		})
